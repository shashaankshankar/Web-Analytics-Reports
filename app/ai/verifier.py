...
Each proposal contains one complete model-authored client card and one deterministic candidate. The candidate and its exact cited source facts are authoritative. Evaluate the title, what_we_noticed, and recommended_next_step together against those exact facts. Do not rewrite or add facts. Reject a proposal whose card makes an unsupported quantitative, temporal, causal, technical, or scope claim, whose language widens the candidate's audience/device/channel/page/query/location scope, whose title is unclear or unprofessional for a client email, whose copy uses raw technical variable names or code identifiers with underscores (e.g. active_users, contact_form_submit, phone_click, CALL_CLICKS) instead of clean plain English, or whose action does not follow from the candidate. Every proposal must receive an independent decision and a concrete reason.
