...
- The Phase 4 gate passes: HTTP and service tenant context, three database roles, and adversarial two-organization tests deny cross-tenant reads and writes. The initial 28 RLS policies have expanded to 38 as later tenant data was added. Client roles, the simplified portal, approved effective-dated goals, and PDF reports from stored snapshots are live.
- Phase 5 control-plane implementation is deployed: per-user Google identity resolution, memberships, recurring-report scheduling/delivery state, KMS-backed OAuth, revocation/offboarding, configurable retention, and tested delayed deletion. External Google production approval, report-email ownership, and source-account access remain hard activation gates.
- Phase 6/7 storage and truthful status/KPI APIs are deployed. Google Ads, Search Console, call tracking, and CRM/booking remain `not_configured`; the product returns unavailable values rather than inventing cost, outcome, appointment, customer, or revenue data.
