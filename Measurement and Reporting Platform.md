...
- Phase 6/7 storage, approval, secret-resolution, scheduling, ingestion, provenance, and truthful reporting APIs are deployed. Google Ads and Search Console have an isolated daily worker path; call tracking and CRM/booking have privacy-validated, idempotent outcome ingestion. Google Ads, Search Console, call tracking, and CRM/booking remain `not_configured`; the product returns unavailable values rather than inventing cost, outcome, appointment, customer, or revenue data.
