...
- Service health: `GET /health` is process liveness; the internal `GET /healthz` probe and authenticated `GET /ready` are database-aware readiness checks. None proves current GA4 access.
...
The live FastAPI routes use literal IDs in the path and an optional fixed `period` query value (`7d`, `28d`, `90d`, `this_month`, or `last_month`). `GET /health` is process liveness; `GET /healthz` and authenticated `GET /ready` fail closed on database or migration unavailability. `GET /docs` is the interactive OpenAPI test surface; reporting endpoints remain protected by Cloud Run IAM and tenant authorization.
