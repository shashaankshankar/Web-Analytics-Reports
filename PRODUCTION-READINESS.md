...
| --- | --- | --- |
| Reporting service | Live, private, and hardened | Cloud Run revision `measurement-reporting-platform-00047-ftn` serves 100% of traffic; authenticated readiness returns 200 and unauthenticated requests return 403. Its distroless Debian 13 image was scanned after deployment and reports zero critical/high findings and zero fixable critical/high findings. |
...
The local suite passes 58 tests, including the strict trace-label allowlist, all seven named operator-alert decisions, the enabled-Testing/false-public-approval OAuth boundary, encrypted authorization-state persistence, and callback-only route isolation. The authenticated production dashboard includes a read-only OAuth connection control; revision `00047-ftn` returns authenticated readiness with live reporting and database status `ok`, and OAuth status reports zero stored connections after the mismatched test grant was revoked.

After the tracing deployment, one signed schedule created all five period tasks. Five worker requests completed with HTTP 200 on revision `00047-ftn`, the queue drained to zero, and the database reached 54 successful executions with no queued or failed jobs. `measurement-report-dispatch` remained paused and was not invoked. Cloud Trace contains sanitized `POST /internal/schedule`, `POST /internal/sync`, `GET /ready`, and callback spans whose labels omit URLs, query strings, headers, bodies, and exception messages. On-Demand scan `72e10649-0ae6-4260-89c3-2b9f9c4d76ac` for image tag `362d4d7` found 50 low/medium/minimal/unknown findings, zero critical/high findings, and zero fixable critical/high findings.
