...
| --- | --- | --- |
| Reporting service | Live, private, and hardened | Cloud Run revision `measurement-reporting-platform-00041-n24` serves 100% of traffic; authenticated readiness returns 200 and unauthenticated requests return 403. Its distroless Debian 13 image was scanned after deployment and reports zero critical/high findings and zero fixable critical/high findings. |
...
| OAuth | Live in Google Testing mode | The external Google Auth Platform app, one approved operator test user, a web client, version-pinned Secret Manager credentials, KMS, PKCE, signed state, `analytics.readonly`, refresh-token encryption, revocation, re-consent, and offboarding are active. A separate public callback-only service exposes health and the callback while returning 404 for root, docs, readiness, agency, and API routes; the reporting service remains private. The approved user grant completed, the refresh token is KMS-encrypted, and property discovery returns one read-only property. It is not the approved House of Dental property, so the connection remains `pending_approval` and unassigned. Public publishing remains gated on approved platform legal pages, an owned authorized domain, Google verification, and an explicit production decision. |
...
The local suite passes 50 tests, including the enabled-Testing/false-public-approval OAuth boundary, encrypted authorization-state persistence, and callback-only route isolation. The authenticated production dashboard includes a read-only OAuth connection control; revision `00041-n24` returns authenticated readiness with live reporting and database status `ok`.
